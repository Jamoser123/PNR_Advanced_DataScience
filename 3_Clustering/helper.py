...
def plot_projection_grid(embeddings, dataset_name="mnist", figsize=(14, 10), legend_subplot=None):
    ...
    Args: 
       embeddings (List[Tuple[np.ndarray, np.ndarray, str]]): 
        ...
        dataset_name (str, optional): 
            ...
            Supported: 'mnist', 'breast_cancer', 'heart_disease', 'auto'. Defaults to "mnist".
    ...
    # Collect all labels to determine consistent mapping
    all_labels = []
    for _, y, _ in embeddings:
        if y is not None:
            all_labels.extend(y.tolist() if hasattr(y, 'tolist') else list(y))
    
    # Handle categorical labels by creating consistent mapping
    if len(all_labels) > 0:
        # Check if labels are categorical (strings or mixed types)
        sample_labels = all_labels[:min(100, len(all_labels))]  # Sample for efficiency
        is_categorical = any(isinstance(label, str) for label in sample_labels)
        
        if is_categorical or dataset_name == "auto":
            # Create consistent mapping for all categorical labels
            ...
            convert_labels = lambda y: y  # No conversion needed
    else:
        unique_labels = []
        convert_labels = lambda y: y
    
    # Determine colormap based on number of unique labels
    ...
    # Create legend handles based on dataset or auto-detection
    handles = None
    legend_title = None
    ...
    if dataset_name == "mnist":
        ...
        # Auto-generate handles for categorical data
        handles = [plt.Line2D([], [], marker="o", linestyle="", 
                             color=cmap(i), label=str(label)) 
                  for i, label in enumerate(unique_labels)]
    ...
    for idx, ((X, y, title), ax) in enumerate(zip(embeddings, axes), 1):
        ...
        if y is None:
            ...
            y_numeric = convert_labels(y)
            sc = ax.scatter(X[:, 0], X[:, 1], c=y_numeric, s=size, cmap=cmap, alpha=0.5)
        ...
        if (legend_subplot == idx or (legend_subplot is None and idx == 1)) and handles is not None:
            ...
            if len(handles) > 15:
                displayed_handles = handles[:15]
                displayed_handles.append(plt.Line2D([], [], marker="o", linestyle="", 
                                                   color='gray', label='...'))
                ax.legend(handles=displayed_handles, title=f"{legend_title} (showing first 15)", 
    ...
    for cluster_id in unique_clusters:
        ...
        for col in X.columns:
            if pd.api.types.is_numeric_dtype(X[col]):
                ...
                cluster_info.append(f"{mean_val:.2f}")
            else:
                # For categorical columns, find most common value with count
                value_counts = cluster_data[col].value_counts()
                if len(value_counts) > 0:
                    mode_val = value_counts.index[0]
                    mode_count = value_counts.iloc[0]
                    percentage = (mode_count / cluster_size) * 100
                    cluster_info.append(f"{mode_val} ({mode_count}/{cluster_size}, {percentage:.1f}%)")
                else:
                    cluster_info.append("N/A")
        
        interpretation[f'Cluster {cluster_id}'] = cluster_info
    
    # Add cluster sizes
    cluster_sizes = []
    for cluster_id in unique_clusters:
        size = np.sum(cluster_labels == cluster_id)
        cluster_sizes.append(f"n={size}")
    
    interpretation.loc['Cluster Size'] = cluster_sizes
    
    print("Cluster Characteristics:")
    print("Numeric: mean values | Categorical: mode (count/total, %)")
    print("=" * 80)
...
def plot_kmeans_clusters(X, labels, centroids=None, title="K-Means Clustering Result"):
    """
...
def plot_dbscan_grid(dataset, eps_values, min_samples_values, X_reduced=None):
    """
    ...
    Args:
        dataset (np.ndarray): The input data to cluster, expected shape (n_samples, n_features).
        ...
        X_reduced (np.ndarray, optional): 2D data for plotting, expected shape (n_samples, 2). 
            If None, uses the original dataset for plotting. Defaults to None.
    ...
    # Use X_reduced for plotting if provided, otherwise use original dataset
    plot_data = X_reduced if X_reduced is not None else dataset
    
    # Ensure plot_data is 2D for visualization
    if plot_data.shape[1] != 2:
        raise ValueError(f"Plot data must be 2D, got shape {plot_data.shape}")
    ...
    for i, min_samples in enumerate(min_samples_values):
        for j, eps in enumerate(eps_values):
            ...
            # Perform DBSCAN on original dataset
            ...
            # Draw epsilon circles on the plot data
            ...
            # Plot using the plot_data (could be reduced dimensions)
            ax.scatter(plot_data[:, 0], plot_data[:, 1], s=50, color=colors[y_pred_2], 
                      zorder=10, edgecolor='black', lw=0.5)

            plt.xticks(())
            plt.yticks(())

            # Calculate the limits for the plot based on plot_data
